...
export function getPhoneSetupCopy(state: PhoneSetupState): PhoneSetupCopy {
  const copies: Record<PhoneSetupState, PhoneSetupCopy> = {
    ...
    payment_verification_pending: {
      ...
      explanation: 'We are waiting for billing to confirm your trial. This usually updates within a minute. Live answering stays off until verification is complete.',
      primaryLabel: 'Refresh status',
      primaryTarget: 'refresh',
      secondaryLabel: 'Open Billing',
      secondaryTarget: '/user/billing',
      blockingReason: 'This usually updates shortly after checkout confirmation.',
    },
    payment_method_required: {
    ...
    billing_issue: {
      ...
      explanation: 'Your billing status needs attention. RingBooker will not answer forwarded live calls until billing is resolved.',
