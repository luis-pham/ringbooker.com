...
.onb-sheet{background:#fff;border-radius:20px 20px 0 0;padding:20px 20px 40px;width:100%;box-sizing:border-box;font-family:inherit}
.onb-sheet-handle{width:36px;height:4px;background:#e5e7eb;border-radius:2px;margin:0 auto 20px}
.onb-sheet-title{font-size:15px;font-weight:600;color:#111;margin-bottom:16px;font-family:inherit}
.onb-sheet-field{margin-bottom:0}
.onb-sheet-label{font-size:11px;font-weight:500;color:#6b7280;margin-bottom:5px;font-family:inherit}
.onb-sheet-input{width:100%;font-size:16px;padding:12px 14px;border:1px solid #e5e7eb;border-radius:10px;font-family:inherit;color:#111;outline:none;box-sizing:border-box}
.onb-sheet-input:focus{border-color:#7c3aed}
.onb-sheet-actions{display:flex;gap:10px;margin-top:16px}
.onb-sheet-cancel{align-items:center;box-sizing:border-box;display:inline-flex;flex:1 1 0;justify-content:center;min-height:48px;min-width:0;padding:13px;border:1px solid #e5e7eb;border-radius:10px;font-size:14px;color:#6b7280;background:#fff;cursor:pointer;font-family:inherit}
.onb-sheet-save{align-items:center;box-sizing:border-box;display:inline-flex;flex:1 1 0;justify-content:center;min-height:48px;min-width:0;padding:13px;border:none;border-radius:10px;font-size:14px;font-weight:500;color:#fff;background:#111;cursor:pointer;font-family:inherit}
...
.onb-add-service-group-btn{width:100%;padding:14px;margin-top:8px;border:1.5px dashed #e5e7eb;border-radius:12px;background:transparent;font-size:14px;font-weight:500;color:#7c3aed;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;font-family:inherit;box-sizing:border-box}
...
.preset-row{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 14px}.onb-chips-tier2{margin-bottom:14px}.onb-chips-show-more-btn{font-size:12px;color:#7c3aed;background:none;border:none;cursor:pointer;padding:4px 0;font-family:inherit;margin-top:4px;text-align:left}.preset-chip{border:1px solid #e2e8f0;border-radius:999px;background:#fff;padding:8px 14px;font-size:13px;font-weight:500;cursor:pointer;box-shadow:none;transition:border-color .15s ease,background .15s ease}.preset-chip:hover{border-color:#c4b5fd;background:#faf5ff}.preset-chip:focus-visible{outline:2px solid #7c3aed;outline-offset:2px}
      ...
      <div className="onb-step-confirm">
        ...
        <div className="profile-review-grid">
          ...
          {profileCard(
            ...
            address,
            <textarea
              className="onb-step2-address-textarea"
              value={address}
              onChange={(event) => {
    ...
    const renderServiceEditor = (group: string, index: number | null, mode: 'inline' | 'sheet') => {
      ...
      return (
        <div className={mode === 'inline' ? 'onb-service-edit-row' : 'onb-service-sheet-fields'} onKeyDown={handleServiceEditorKeyDown}>
          {mode === 'inline' ? <div className="onb-service-edit-main-row">{fields}</div> : fields}
            ...
            <div className="onb-service-variants-editor">
              ...
              {draft.variants.map((variant, variantIndex) => (
                <div className="onb-service-variant-edit" key={`${variant.label}-${variantIndex}`}>
                  <div className="onb-svc-var-row1">
                    <input
                      value={variant.label}
                  ...
                  <div className="onb-svc-var-row2">
                    <div className="onb-service-price-input">
                      <span>$</span>
                      <input
                        type="number"
      ...
      return (
        <div className="service-group-card" key={group} data-service-group={group}>
          <div className="service-group-head">
            ...
            <button
              type="button"
              className="onb-group-add-header-desktop cursor-pointer text-sm font-medium text-purple-700 py-2"
              onClick={() => addBlankService(group, 'inline')}
            >
              <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {!collapsed ? (
            <div className="service-group-body">
              ...
              <button
                type="button"
                className="onb-group-add-service-mobile mt-3 flex w-full cursor-pointer items-center justify-center gap-1.5 py-2 text-sm font-medium text-purple-700"
                onClick={() => addBlankService(group, 'sheet')}
              >
                <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 5v14M5 12h14" />
                </svg>
