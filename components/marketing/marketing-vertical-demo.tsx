...
function normalizeImportedSiteUrl(raw: string): string | undefined {
  const t = raw.trim();
  if (!t) return undefined;
  try {
    return new URL(t.includes('://') ? t : `https://${t}`).toString();
  } catch {
    return undefined;
  }
}
...
const collapsedTurnstileStyle = { marginBottom: 0, minHeight: 0 } as const;
...
export function DemoExperience({
  ...
  function validate(): string[] {
    ...
    if (turnstileSiteKey && !captchaToken) errs.push('Please wait a moment for verification to complete, then try again.');
    return errs;
  }
  ...
  function buildDemoPayload() {
    return {
      ...
      importedSiteUrl: sitePhase === 'ready' ? normalizeImportedSiteUrl(siteUrl) : undefined,
