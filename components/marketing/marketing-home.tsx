...
:root{
  ...
  --home-shadow-soft:var(--mk-card-shadow,0 1px 2px rgba(17,24,39,.04));
  --home-shadow-hover:var(--mk-card-shadow-hover,0 4px 14px -6px rgba(17,24,39,.08));
  --home-card-border-hover:var(--mk-card-border-hover,rgba(167,139,250,.42));
...
.hero-stat-label{font-size:13px;color:var(--text-dark);font-weight:500;line-height:1.4}
	...
	.hero-btns .btn-hero-live{background:#0d0d0d;color:#fff;padding:15px 32px;border-radius:var(--r-pill);font-size:var(--mk-btn-lg);font-weight:500;text-decoration:none;display:inline-flex;align-items:center;gap:10px;box-shadow:0 8px 24px rgba(0,0,0,.18),0 2px 8px rgba(0,0,0,.1);border:none;transition:transform .15s,background .2s,box-shadow .2s}
	.hero-btns .btn-hero-live:hover{background:#1a1a1a;transform:translateY(-1px);box-shadow:0 12px 32px rgba(0,0,0,.22),0 4px 12px rgba(0,0,0,.12)}
	...
	/* ─── MISSED-CALL / PROBLEM (layout only; tokens match site) ─── */
	.leak-section{position:relative;margin-top:-34px;padding:96px 48px 80px;background:linear-gradient(180deg,rgba(249,250,251,0) 0%,#fcfbff 26%,#F9FAFB 100%)}
	.leak-inner,.compare-inner{max-width:1100px;margin:0 auto}
	.leak-intro{margin-bottom:8px}
	.leak-intro .sec-label,.leak-intro .sec-title,.leak-intro .sec-sub{text-align:left;margin-left:0;margin-right:0}
	.leak-intro .sec-title{max-width:none;margin-bottom:14px}
	.leak-intro .sec-title em{font-style:italic;font-weight:500;color:var(--purple-dark)}
	.leak-intro .sec-sub{max-width:44ch;margin-bottom:44px;line-height:1.65}
	.leak-body{display:grid;grid-template-columns:minmax(0,1.08fr) minmax(0,.92fr);gap:36px 64px;align-items:start}
	.leak-list{display:flex;flex-direction:column;gap:0}
	.leak-item{display:grid;grid-template-columns:3.5rem minmax(0,1fr);gap:16px 26px;padding:20px 0;border-bottom:1px solid var(--border)}
	.leak-item:first-child{padding-top:0}
	.leak-item:last-child{border-bottom:none;padding-bottom:0}
	.leak-num{font-size:clamp(1.75rem,2.8vw,2.35rem);font-weight:500;line-height:1;color:#d1d5db;letter-spacing:-.04em;padding-top:1px;font-variant-numeric:tabular-nums}
	.leak-item h3{font-size:16px;font-weight:500;line-height:1.35;letter-spacing:-.25px;margin-bottom:8px;text-align:left;color:var(--text-dark)}
	.leak-item > div > p{font-size:15px;color:var(--text-desc);line-height:1.6;text-align:left;margin-bottom:10px;max-width:46ch}
	...
	.leak-revenue-kicker{font-size:var(--mk-eyebrow);font-weight:600;color:var(--text-dark);letter-spacing:var(--mk-eyebrow-ls);text-transform:uppercase;margin-bottom:10px;text-align:left;opacity:1}
	.leak-revenue-note{font-size:13px;color:var(--text-light);margin-bottom:20px;line-height:1.5;text-align:left}
	.leak-revenue-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px 16px;align-items:center;margin-bottom:18px}
	.leak-revenue-row:last-of-type{margin-bottom:0}
	.leak-revenue-label{font-size:15px;color:var(--text-desc);line-height:1.45;grid-column:1;text-align:left}
	.leak-revenue-range{font-size:15px;font-weight:500;color:var(--text-dark);white-space:nowrap;grid-column:2;grid-row:1}
	.leak-revenue-bar{grid-column:1/-1;height:7px;border-radius:999px;background:#f1f5f9;overflow:hidden;margin-top:2px}
	.leak-revenue-bar span{display:block;height:100%;border-radius:999px}
	.leak-revenue-foot{margin-top:24px;padding-top:20px;border-top:1px solid #e5e7eb}
	.leak-revenue-foot > p:first-child{font-size:15px;color:var(--text-desc);margin-bottom:10px;text-align:left}
	.leak-revenue-total{font-size:clamp(1.85rem,3.2vw,2.35rem);font-weight:600;color:var(--text-dark);letter-spacing:-.04em;line-height:1.12;font-variant-numeric:tabular-nums;text-align:left}
	.leak-revenue-total strong{font-weight:600;color:var(--purple-dark)}
	.compare-section{padding:88px 48px 72px;background:linear-gradient(180deg,#F9FAFB 0%,#fff 100%);color:var(--text-dark)}
	.compare-intro{margin-bottom:8px}
	.compare-intro .sec-label,.compare-intro .compare-title,.compare-intro .compare-lead{text-align:left;margin-left:0;margin-right:0}
	.compare-title{
		font-size:var(--mk-section-h2);font-weight:500;line-height:var(--mk-section-h2-lh);
		letter-spacing:var(--mk-section-h2-track);color:var(--text-dark);max-width:20ch;margin-bottom:14px;text-wrap:normal;
	}
	.compare-title em{font-style:italic;font-weight:500;color:var(--purple-dark)}
	.compare-lead{font-size:var(--mk-section-lead);color:var(--text-desc);line-height:var(--mk-section-lead-lh);max-width:44ch;font-weight:400}
	.compare-board{
		display:grid;grid-template-columns:repeat(2,minmax(0,1fr));margin-top:36px;
		border:1px solid var(--border);border-radius:20px;background:#fff;overflow:hidden;
		box-shadow:var(--home-shadow-soft);
	}
	.compare-col{padding:28px 26px 24px}
	.compare-col-bad{background:#fafafa;border-right:1px solid rgba(241,245,249,.95)}
	.compare-col-good{background:linear-gradient(180deg,#fafdfb 0%,#fff 100%)}
	.compare-col-label{
		font-size:11px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;
		margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border);
	}
	.compare-col-bad .compare-col-label{color:#94a3b8}
	.compare-col-good .compare-col-label{color:var(--purple-dark)}
	.compare-lines{list-style:none;display:flex;flex-direction:column;gap:0}
	.compare-line{
		display:flex;align-items:flex-start;gap:12px;padding:14px 0;border-bottom:1px solid rgba(241,245,249,.9);
		font-size:var(--mk-body);line-height:1.58;font-weight:400;
	}
	.compare-line:last-child{border-bottom:none;padding-bottom:0}
	.compare-line:first-child{padding-top:0}
	.compare-col-bad .compare-line{color:var(--text-desc)}
	.compare-col-good .compare-line{color:var(--text-dark)}
	.cmp-icon{width:20px;height:20px;min-width:20px;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:3px}
	.cmp-icon svg{display:block}
	.compare-col-bad .cmp-icon{color:#F87171}
	.compare-col-good .cmp-icon{color:#34D399}

	/* ─── HERO VISUAL ─── */
.hero-visual{position:relative;margin-top:0;padding-top:20px;height:500px;display:flex;align-items:flex-start;justify-content:center;min-width:0}
...
.p-time-txt{font-size:14px;font-weight:500;color:#fff}
.p-icons-txt{font-size:10px;color:rgba(255,255,255,.7);letter-spacing:.5px}

/* voice call screen */
.vc-bg{position:absolute;inset:0;background:#f3f4f6}
...
.logo-item{display:flex;align-items:center;gap:8px;font-size:var(--mk-body-md);font-weight:500;color:#C4C9D4;transition:color .2s;cursor:default}
...
.sec-title{font-size:var(--mk-section-h2);font-weight:500;line-height:var(--mk-section-h2-lh);letter-spacing:var(--mk-section-h2-track);text-align:center;margin-bottom:14px;color:var(--text-dark);max-width:22ch;margin-left:auto;margin-right:auto;text-wrap:balance}
...
.emphasis-5min{color:var(--purple-dark);font-weight:500}

/* ─── CORE COVERAGE (1100px frame, 50/50 rows) ─── */
.features{
	--cov-accent:#EE5D43;
	--cov-peach:#F9E4E1;
	--cov-sage:#E1ECE6;
	...
	background:linear-gradient(180deg,rgba(249,250,251,0) 0%,#fcfbff 26%,#F9FAFB 100%);
}
...
.coverage-title em{font-style:italic;font-weight:500;color:var(--purple-dark)}
.coverage-lead{font-size:var(--mk-section-lead);color:var(--text-desc);line-height:var(--mk-section-lead-lh);max-width:40ch;font-weight:400}
.coverage-rows{display:flex;flex-direction:column;gap:0;border-top:1px solid var(--border)}
.coverage-row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0;align-items:stretch;min-height:300px}
...
.coverage-copy-inner{width:100%;max-width:36ch}
.coverage-item-kicker{font-size:11px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--cov-accent);margin-bottom:14px}
.coverage-copy h3{
	font-size:clamp(1.35rem,2.4vw,1.75rem);font-weight:500;line-height:1.18;letter-spacing:-.4px;
...
.coverage-visual-peach{background:var(--cov-peach)}
.coverage-visual-sage{background:var(--cov-sage)}
.coverage-visual-sand{background:var(--cov-sand)}
.coverage-visual-num{
	position:absolute;top:clamp(12px,2vw,20px);right:clamp(16px,3vw,28px);
	font-size:clamp(3.5rem,8vw,5.5rem);font-weight:500;line-height:1;
	color:rgba(17,24,39,.06);letter-spacing:-.05em;pointer-events:none;user-select:none;font-variant-numeric:tabular-nums;
}
.coverage-panel{
	position:relative;z-index:1;width:min(100%,260px);background:#fff;border-radius:16px;padding:16px 16px 14px;
	box-shadow:var(--home-shadow-soft);
	border:1px solid var(--border);
}
.coverage-panel-title{font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8;margin-bottom:12px}
.coverage-call-row{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f1f5f9}
.coverage-call-row:last-child{border-bottom:none;padding-bottom:0}
.coverage-call-row:first-child{padding-top:0}
.coverage-call-ico{
	width:28px;height:28px;border-radius:8px;background:#f8fafc;border:1px solid #e5e7eb;
	display:flex;align-items:center;justify-content:center;color:#64748b;flex-shrink:0;
}
.coverage-call-ico .home-line-icon{width:14px;height:14px}
.coverage-call-main{flex:1;min-width:0}
.coverage-call-label{font-size:13px;font-weight:500;color:var(--text-dark);line-height:1.3}
.coverage-call-time{font-size:12px;color:var(--text-desc);line-height:1.35}
.coverage-badge{font-size:9px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;padding:4px 7px;border-radius:999px;white-space:nowrap;flex-shrink:0}
.coverage-badge-answered{color:#047857;background:#ecfdf5}
.coverage-badge-missed{color:#B45309;background:#fffbeb}
.coverage-badge-ready{color:#1d4ed8;background:#eff6ff}
.coverage-badge-delivered{color:#047857;background:#ecfdf5}
.coverage-status-row{display:flex;justify-content:space-between;align-items:baseline;gap:12px;padding:7px 0;border-bottom:1px solid #f1f5f9;font-size:12px}
.coverage-status-row:last-child{border-bottom:none}
.coverage-status-label{color:var(--text-desc);font-weight:400}
.coverage-status-value{color:var(--text-dark);font-weight:500;text-align:right}
.coverage-status-highlight .coverage-status-value{color:#047857}
.coverage-msg-row{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid #f1f5f9}
.coverage-msg-row:last-child{border-bottom:none}
.coverage-msg-text{font-size:12px;color:var(--text-desc);line-height:1.45;flex:1;min-width:0}
.coverage-msg-text strong{color:var(--text-dark);font-weight:500}
...
.fi-p{color:var(--purple-dark);background:linear-gradient(180deg,#faf9ff,#f5f3ff);border-color:rgba(196,181,253,.35)}
...
.deep-s1 .d-text h2{font-size:clamp(20px,2vw,27px);font-weight:500;line-height:1.2;letter-spacing:-.6px;margin-bottom:10px;color:var(--text-dark)}
.deep-s1 .d-text p{font-size:var(--mk-body);color:var(--text-desc);line-height:1.68;margin-bottom:16px;font-weight:400}
.deep-s2-outer .d-text h2{font-size:clamp(20px,2vw,27px);font-weight:500;line-height:1.2;letter-spacing:-.6px;margin-bottom:16px;color:var(--text-dark)}
...
.chart-big{font-size:34px;font-weight:500;letter-spacing:-.8px;color:var(--text-dark)}
...
.convo-avatar{width:38px;height:38px;border-radius:50%;border:2.5px solid #fff;margin-right:-10px;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:500;color:#fff}
...
.convo-quote{font-size:20px;font-weight:500;color:var(--text-dark);line-height:1.3;margin-bottom:18px;letter-spacing:-.35px}
.convo-wave{height:50px;width:100%;opacity:.15}
.convo-wave path{fill:none;stroke:var(--text-dark);stroke-width:1.5px}
/* ─── Industries metrics (inside #industries) ─── */
...
.metrics-value{font-size:34px;font-weight:500;letter-spacing:-.8px;color:var(--text-dark);line-height:1.1;margin:0 0 8px}
.metrics-label{font-size:15px;font-weight:500;color:var(--text-dark);line-height:1.35;margin-bottom:6px}
.metrics-sublabel{font-size:13px;color:var(--text-desc);line-height:1.5;font-weight:400;margin:0}
.metrics-lang-tags{
...
@media(max-width:767px){
  ...
  .metrics-cell:last-child{border-bottom:none}
  .metrics-value{
    grid-column:2;grid-row:1/-1;align-self:center;
    font-size:36px;font-weight:500;letter-spacing:-.6px;margin:0;
  }
  .metrics-label{
    grid-column:1;grid-row:1;
    font-size:14px;font-weight:500;color:var(--text-desc);margin:0;line-height:1.35;
  }
  .metrics-sublabel{
    grid-column:1;grid-row:2;
    font-size:12px;color:var(--text-light,#94a3b8);line-height:1.45;margin:0;
  }
  .metrics-cell--langs .metrics-label{grid-row:1;margin-bottom:0}
  .metrics-lang-tags{justify-content:flex-start;grid-column:1;grid-row:2;margin:8px 0 0}
  .legacy-marketing .industries .metrics-label{font-size:14px !important}
  .legacy-marketing .industries .metrics-sublabel{font-size:12px !important}
  .legacy-marketing .industries .metrics-value{font-size:36px !important}
}
...
/* ─── PRICING (mock layout, site light + purple tokens) ─── */
.pricing{padding:88px 48px 72px;background:var(--bg-gray);color:var(--text-dark)}
.pricing-inner{max-width:1100px;margin:0 auto}
.pricing-intro{margin-bottom:8px}
.pricing-intro .sec-label,.pricing-intro .pricing-title,.pricing-intro .pricing-lead{text-align:center;margin-left:auto;margin-right:auto}
.pricing-title{
	font-size:var(--mk-section-h2);font-weight:500;line-height:var(--mk-section-h2-lh);
	letter-spacing:var(--mk-section-h2-track);color:var(--text-dark);max-width:18ch;margin-bottom:14px;text-wrap:balance;
}
.pricing-title em{font-style:italic;font-weight:500;color:var(--purple-dark)}
.pricing-lead{font-size:var(--mk-section-lead);color:var(--text-desc);line-height:var(--mk-section-lead-lh);max-width:44ch;font-weight:400;margin-bottom:32px}
.price-toggle{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;margin-bottom:40px}
...
.plan:hover{transform:translateY(-1px);box-shadow:var(--home-shadow-hover);border-color:var(--home-card-border-hover)}
.plan.star{padding-top:44px;background:linear-gradient(180deg,#faf9ff 0%,#fff 85%);border-color:rgba(167,139,250,.55);box-shadow:var(--home-shadow-soft)}
.plan.star:hover{border-color:rgba(167,139,250,.65);box-shadow:var(--home-shadow-hover)}
.plan-badge{
	position:absolute;top:18px;right:18px;left:auto;transform:none;
	background:var(--purple);color:#fff;font-size:var(--mk-badge);font-weight:600;
	letter-spacing:var(--mk-eyebrow-ls);text-transform:uppercase;padding:5px 12px;border-radius:var(--r-pill);
	white-space:nowrap;box-shadow:0 4px 12px rgba(124,58,237,.2);
}
.plan-name{font-size:var(--mk-card-title);font-weight:600;margin-bottom:8px;color:var(--text-dark)}
.plan-desc{font-size:var(--mk-caption);color:var(--text-desc);margin-bottom:16px;line-height:1.55;font-weight:400}
.plan-price{font-size:38px;font-weight:500;letter-spacing:-1.5px;margin-bottom:5px;color:var(--text-dark)}
.plan-price-custom{font-size:var(--mk-card-title);font-weight:600;letter-spacing:-.2px;line-height:1.35;color:var(--text-dark)}
.plan-price .plan-price-period{font-size:var(--mk-body);font-weight:500;color:var(--text-gray);letter-spacing:0}
.plan-price .plan-price-billed{display:block;font-size:13px;font-weight:500;color:var(--text-desc);letter-spacing:0;line-height:1.45;margin-top:4px}
.plan-cta-subnote{font-size:12px;color:var(--text-desc);text-align:center;margin:8px 0 0;font-weight:400;line-height:1.45}
.plan-div{height:1px;background:var(--border);margin:18px 0 20px}
...
.plan-btn{
	width:100%;padding:12px;border-radius:var(--r-pill);font-size:var(--mk-btn-sm);font-weight:600;
	text-align:center;display:block;transition:all .2s;cursor:pointer;border:none;font-family:inherit;
...
.pricing-foot{margin-top:28px;font-size:13px;line-height:1.6;color:var(--text-desc);text-align:center;font-weight:400}
/* ─── HOW IT WORKS (setup rail) ─── */
...
.steps-intro .sec-label,.steps-intro .steps-title,.steps-intro .steps-copy{text-align:left;margin-left:0;margin-right:0}
.steps-title{font-size:var(--mk-section-h2);font-weight:500;line-height:var(--mk-section-h2-lh);letter-spacing:var(--mk-section-h2-track);color:var(--text-dark);max-width:20ch;margin-bottom:14px;text-wrap:balance}
.steps-title em{font-style:italic;font-weight:500;color:var(--purple-dark)}
.steps-copy{font-size:var(--mk-section-lead);color:var(--text-desc);line-height:var(--mk-section-lead-lh);max-width:44ch;font-weight:400}
.steps-shell{position:relative;--step-marker-size:50px}
.steps-rail{
	position:absolute;left:16.666%;right:16.666%;top:calc(var(--step-marker-size)/2);height:0;z-index:0;
	border-top:1px dashed #d1d5db;
}
.steps-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:28px;position:relative}
...
.step-marker{
	width:var(--step-marker-size);height:var(--step-marker-size);border-radius:999px;
	border:1.5px solid var(--purple);background:#fff;color:var(--purple-dark);
	display:flex;align-items:center;justify-content:center;
	font-size:17px;font-weight:500;margin:0 auto 22px;position:relative;z-index:1;
}
.step-card h3{font-size:var(--mk-card-title);font-weight:500;letter-spacing:-.2px;line-height:1.35;margin-bottom:10px;color:var(--text-dark);max-width:280px;margin-left:auto;margin-right:auto}
.step-card p{font-size:var(--mk-body);color:var(--text-desc);line-height:var(--mk-body-lh);max-width:300px;margin:0 auto;font-weight:400}

/* ─── INDUSTRIES (5-card row) ─── */
.industries{padding:88px 48px;background:linear-gradient(180deg,#F9FAFB 0%,#fff 100%)}
.industries-inner{max-width:1100px;margin:0 auto}
.industries-intro{margin-bottom:8px}
.industries-intro .sec-label,.industries-intro .industries-title,.industries-intro .industries-lead{text-align:left;margin-left:0;margin-right:0}
.industries-title{
	font-size:var(--mk-section-h2);font-weight:500;line-height:var(--mk-section-h2-lh);
	letter-spacing:var(--mk-section-h2-track);color:var(--text-dark);max-width:22ch;margin-bottom:14px;text-wrap:balance;
}
...
.home-carousel-nav-btn{
  ...
  display:flex;align-items:center;justify-content:center;font-size:19px;font-weight:500;cursor:pointer;box-shadow:0 6px 18px rgba(124,58,237,.10);
}
...
.flow-dot{width:34px;height:34px;min-width:34px;border-radius:12px;background:var(--purple-ultra);color:var(--purple-dark);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:500;border:1px solid rgba(196,181,253,.35)}
...
.scope-icon{width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:500;flex-shrink:0;margin-top:1px}
...
.user-copy h2{text-align:left;font-size:clamp(28px,3.6vw,44px);font-weight:500;line-height:1.12;letter-spacing:-1.1px;margin-bottom:14px;color:var(--text-dark)}
...
.user-shot{border-radius:28px;background:linear-gradient(180deg,#ffffff,#f8fafc);padding:14px;box-shadow:var(--home-shadow-soft);border:1px solid var(--border)}
...
.user-image-badge{display:inline-flex;align-items:center;gap:8px;background:#fff;border:1px solid rgba(139,92,246,.20);border-radius:999px;padding:8px 12px;font-size:11px;font-weight:500;letter-spacing:.08em;text-transform:uppercase;color:var(--purple-dark);margin-bottom:16px;box-shadow:var(--home-shadow-soft)}
.user-image-icon{width:74px;height:74px;border-radius:22px;background:linear-gradient(135deg,#ede9fe,#f5f3ff);border:1px solid #ddd6fe;display:flex;align-items:center;justify-content:center;color:var(--purple-dark);font-size:30px;font-weight:600;margin-bottom:16px}
.user-image-title{font-size:20px;font-weight:600;letter-spacing:-.5px;margin-bottom:8px}
...
/* ─── CTA BANNER (purple gradient, centered layout) ─── */
.cta-outer{padding:0 48px 72px;display:flex;justify-content:center}
.cta-inner{width:100%;max-width:1100px}
.cta-banner{
	position:relative;overflow:hidden;
	border-radius:var(--r-lg);background:linear-gradient(125deg,#6D28D9 0%,#8B5CF6 55%,#A78BFA 100%);
	padding:clamp(48px,6vw,72px) clamp(28px,5vw,56px);
	display:flex;justify-content:center;align-items:center;text-align:center;
	box-shadow:0 20px 40px -8px rgba(91,33,182,.28),0 8px 16px -6px rgba(91,33,182,.18);
}
.cta-banner::before{content:"";position:absolute;right:-30px;top:-40px;width:280px;height:280px;background:rgba(255,255,255,.07);border-radius:50%;pointer-events:none}
.cta-stack{position:relative;z-index:1;max-width:720px;width:100%}
.cta-title{
	font-size:clamp(1.75rem,3.2vw,2.5rem);font-weight:500;color:#fff;
	letter-spacing:-.03em;line-height:1.15;margin-bottom:16px;text-wrap:balance;
}
.cta-title em{font-style:italic;font-weight:500;color:#EDE9FE}
.cta-lead{
	font-size:15px;color:rgba(255,255,255,.78);line-height:1.65;
	margin:0 auto 28px;max-width:52ch;font-weight:400;
}
...
.cta-banner .btn-hero-live .btn-hero-live-phone path{fill:#FACC15}
.cta-banner .btn-hero-live .btn-hero-live-arrow{color:#fff}
...
.footer-brand{display:flex;align-items:center;gap:10px;font-weight:600;font-size:17px;margin-bottom:14px}
...
.footer-col h4{font-size:11.5px;font-weight:500;color:var(--text-dark);text-transform:uppercase;letter-spacing:.07em;margin-bottom:16px}
...
/* ─── RESPONSIVE ─── */
@media(max-width:960px){
  .leak-body{grid-template-columns:1fr;gap:48px;align-items:start}
  .leak-item{padding:22px 0;gap:18px}
  .leak-card.leak-revenue{height:auto}
  .leak-revenue{max-width:520px}
  .leak-card.leak-revenue{padding:32px 26px 28px}
  .coverage-row,.coverage-row-reverse{grid-template-columns:1fr;min-height:0}
  .coverage-row-reverse .coverage-copy,.coverage-row-reverse .coverage-visual{order:unset}
  .coverage-copy{padding:40px 28px}
  .coverage-visual{min-height:280px;padding:36px 24px}
  .coverage-panel{width:min(100%,320px)}
  .compare-board{grid-template-columns:1fr}
  .compare-col-bad{border-right:none;border-bottom:1px solid rgba(241,245,249,.95)}
  .deep-s1,.deep-s2-outer,.price-grid,.footer-grid{grid-template-columns:1fr}
  .cta-banner{padding:40px 24px}
  .cta-row{flex-direction:column;align-items:stretch;gap:14px}
  .cta-banner .btn-hero-live,.cta-banner .btn-outline.btn-hero-trial{justify-content:center;width:100%}
...
@media(max-width:640px){
  ...
  .hero-btns .btn-hero-trial{width:100%;justify-content:center}
  .features,.compare-section,.pricing{padding-top:var(--mk-space-section-y-mobile);padding-bottom:64px}
  .steps-section,.industries{padding-top:var(--mk-space-section-y-mobile);padding-bottom:64px}
  .sec-sub{margin-bottom:32px}
  .steps-intro{margin-bottom:28px}
  .steps-grid{gap:24px}
