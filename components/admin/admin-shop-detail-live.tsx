...
export function AdminShopDetailLive() {
  ...
  useEffect(() => {
    if (!shopId) return;
    const shopChanged = prevShopIdRef.current !== shopId;
    prevShopIdRef.current = shopId;
    if (shopChanged && callsPage !== 1) {
      setCallsPage(1);
      return;
    }

    let cancelled = false;
    const pageToLoad = shopChanged ? 1 : callsPage;
    void fetch(`/api/backend/admin/shops/${shopId}?callsPage=${pageToLoad}`)
      ...
      .then((body) => {
        ...
        setError(null);
        setShop(body.shop);
        const newCalls = body.recentCalls ?? [];
        setRecentCalls(newCalls);
        setCallsPagination(body.callsPagination ?? { page: pageToLoad, pageSize: 20, total: newCalls.length });
        setActiveCall((prev) => {
          if (!prev) return newCalls[0] ?? null;
          const still = newCalls.find((c) => c.providerCallId === prev.providerCallId && c.shopId === prev.shopId);
          return still ?? newCalls[0] ?? null;
        });
      })
      .catch(() => {
  ...
  return (
    <AdminLayout styles={[...adminShopDetailStyles, ...adminSidebarAddonStyles]} scripts={adminShopDetailScripts} scriptPrefix="admin-shop-detail-live" bodyClass="app-body">
                ...
                <div className="card">
                  ...
                  {recentCalls.length === 0 ? (
                    ...
                    <>
                      ...
                      {callsPagination && callsPagination.total > callsPagination.pageSize ? (
                        <div className="top-actions" style={{ marginTop: 14, justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                          <p className="sub" style={{ margin: 0 }}>
                            Page {callsPagination.page} of {Math.max(1, Math.ceil(callsPagination.total / callsPagination.pageSize))}
                            <span style={{ opacity: 0.75 }}> · {callsPagination.total} calls</span>
                          </p>
