...
export function UserLoginForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = (await response.json().catch(() => null)) as { error?: string; onboardingRequired?: boolean } | null;
      if (!response.ok) {
    ...
    <main className={`${styles.page} ${plusJakarta.className}`}>
      ...
      <section className={styles.wrap}>
        <div className={styles.card}>
          ...
          <p className={styles.sub}>Sign in to manage calls, bookings, and your business number.</p>
          ...
          <a href="/api/backend/auth/user/google/start?intent=login" className={`${styles.btn} ${styles.btnSecondary}`}>
            <svg className={styles.googleIcon} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
              <g clipPath="url(#google-login-clip)">
