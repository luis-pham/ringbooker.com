...
function normalizeFooterCtasJson(data: PostFormData) {
  const rows = data.footerCtas.map((row) => ({
    kind: row.kind,
    primaryHref: row.primaryHref.trim(),
    secondaryHref: row.secondaryHref.trim(),
  }));
...
export async function createPost(data: PostFormData): Promise<{ id: string }> {
  ...
  const post = await prisma.post.create({
    data: {
      ...
      metaDescription: parsed.metaDescription.trim() ? parsed.metaDescription.trim() : null,
      seoTitle: parsed.seoTitle.trim() ? parsed.seoTitle.trim() : null,
