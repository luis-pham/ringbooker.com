...
- `PADDLE_TRIAL_CONFIG_VERIFIED=true` only after verifying the Paddle price trial config in sandbox. In production, trial checkout returns 503 (`billing_trial_config_unverified`) until this flag is set, to prevent charging cards immediately instead of at trial end.
...
- `WEBSITE_IMPORT_ENABLED=false` until website import QA passes; set `true` to enable server-side onboarding website suggestions for controlled beta only. Keep `NEXT_PUBLIC_WEBSITE_IMPORT_ACTIVE` aligned for the user-facing onboarding CTA/copy. Current importer classification: Static + Google Places + optional LLM. `GOOGLE_PLACES_API_KEY` is optional and enables Google Places enrichment for Google Maps links and conservative website/name/domain lookup; when unset, import stays static-only. `WEBSITE_IMPORT_LLM_ENABLED=false` keeps structured LLM extraction off by default; when enabled with `OPENAI_API_KEY`, it adds reviewable suggestions only. LLM extraction is capped by a cross-instance daily limit (`WEBSITE_IMPORT_LLM_GLOBAL_DAILY_LIMIT`, default 500; needs `REDIS_URL` to enforce across instances) — past the cap, imports still complete via static + Google Places extraction without LLM enrichment. Website import supports reviewable service variants/options for menus where one service has multiple duration/price choices, for example `Relaxing` with 30/60/90 minute prices. Users must review imported services and variants before saving; extraction is not guaranteed perfect. Import suggestions include secondary Business Knowledge candidates for staff, policies, FAQs, promotions, and booking setup hints. These are not auto-saved into live AI knowledge; they are stored in the `business_knowledge_suggestions` review queue so users can review, edit, apply, or dismiss them later in Business Knowledge. `WEBSITE_IMPORT_MAX_BYTES` caps each fetched page body size. Successful imports are cached in-process for one hour (keyed by normalized URL) so the demo → onboarding handoff and accidental double-submits reuse work; failures are never cached. This is not a full adaptive importer: Jina/Reader and Playwright fallback are not implemented, and users must review before saving. SSRF protection: the crawler resolves DNS and rejects private/loopback/link-local/cloud-metadata targets, then pins the connection to the validated public IP (overriding the socket lookup and re-checking every redirect), which closes the DNS-rebinding gap without a separate proxy. A sandboxed egress firewall is still recommended as defense-in-depth before wide public rollout.
- `PADDLE_API_KEY=...`
- `PADDLE_CLIENT_TOKEN=...`
