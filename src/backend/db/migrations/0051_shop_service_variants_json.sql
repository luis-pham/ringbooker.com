...
alter table shop_services
  drop constraint if exists shop_services_variants_json_array_check;

alter table shop_services
  add constraint shop_services_variants_json_array_check
  check (variants_json is null or jsonb_typeof(variants_json) = 'array');
...
begin
  ...
  select
    ...
    case when jsonb_typeof(service.variants_json) = 'array' then service.variants_json else null end,
