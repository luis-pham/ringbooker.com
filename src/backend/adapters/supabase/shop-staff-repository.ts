...
function toRow(staff: UpsertShopStaff) {
  return {
...
export class SupabaseShopStaffRepository implements ShopStaffRepository {
  ...
  async findByShopId(shopId: string): Promise<ShopStaff[]> {
    const { data, error } = await this.supabase
      .from('shop_staff')
      .select('id,shop_id,name,role,specialties,notes,active,external_provider,external_staff_id,external_metadata,created_at,updated_at')
      .eq('shop_id', shopId)
