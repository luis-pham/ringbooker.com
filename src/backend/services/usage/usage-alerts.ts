...
function buildUsageAlertEmail(params: {
  ...
  if (params.alertType === '80pct_warning') {
    ...
    const text = [
      `Hi ${params.shop.name},`,
      `You've used ${used} of ${limit} captured callers this billing period.`,
      `You have ${remaining} callers remaining before overage charges apply.`,
      `Once you reach ${limit} callers, additional callers will be billed at $0.25 each at the end of your billing period.`,
    ...
    return {
      ...
      input: {
        title: subject,
        previewText: `${used} of ${limit} captured callers used this billing period.`,
        heroTitle: 'Caller usage update',
        heroSubtitleHtml: `<p style="margin:0"><strong>${used}</strong> of <strong>${limit}</strong> callers used this period.</p>`,
        greetingHtml: `<p style="margin:0">Hi ${escapeHtmlText(params.shop.name)},</p>`,
        bodyHtml: [
          `<p style="margin:0 0 12px 0">You've used ${used} of ${limit} captured callers this billing period.</p>`,
          `<p style="margin:0 0 12px 0">You have ${remaining} callers remaining before overage charges apply.</p>`,
          `<p style="margin:0 0 12px 0">Once you reach ${limit} callers, additional callers will be billed at $0.25 each at the end of your billing period.</p>`,
  ...
  const text = [
    `Hi ${params.shop.name},`,
    `You've reached your ${planName} caller limit of ${limit} callers this period.`,
    'Your AI receptionist is still answering calls.',
    `Additional captured callers will be billed at $0.25 each at the end of your billing period on ${formatDate(params.periodEnd)}.`,
    `Current overage: ${overageCallers} callers = $${money(estimatedCents)} estimated`,
    `View usage dashboard: ${dashboard}`,
  ...
  return {
    ...
    input: {
      title: subject,
      previewText: `${planName} caller limit reached; overage now applies.`,
      heroTitle: 'Caller limit reached',
      heroSubtitleHtml: `<p style="margin:0">Additional captured callers are now billed at $0.25 each.</p>`,
      greetingHtml: `<p style="margin:0">Hi ${escapeHtmlText(params.shop.name)},</p>`,
      bodyHtml: [
        `<p style="margin:0 0 12px 0">You've reached your ${escapeHtmlText(planName)} caller limit of ${limit} callers this period.</p>`,
        '<p style="margin:0 0 12px 0">Your AI receptionist is still answering calls.</p>',
        `<p style="margin:0 0 12px 0">Additional captured callers will be billed at $0.25 each at the end of your billing period on ${escapeHtmlText(formatDate(params.periodEnd))}.</p>`,
        `<p style="margin:0">Current overage: ${overageCallers} callers = $${money(estimatedCents)} estimated</p>`,
...
export function buildOverageChargeReceiptEmail(params: {
  ...
  const text = [
    ...
    `Plan:              ${planName}`,
    `Included callers:  ${params.includedCallers}`,
    `Callers used:      ${params.capturedCallers}`,
    `Overage callers:   ${params.overageCallers}`,
    'Overage rate:      $0.25/caller',
  ...
  return {
    ...
    input: {
      title: subject,
      previewText: `Captured caller overage charge: $${money(params.amountCents)}.`,
      heroTitle: 'Overage charge receipt',
      heroSubtitleHtml: `<p style="margin:0">$${money(params.amountCents)} charged for ${params.overageCallers} additional callers.</p>`,
      greetingHtml: `<p style="margin:0">Hi ${escapeHtmlText(params.shop.name)},</p>`,
      bodyHtml: [
        ...
        `<p style="margin:0">Plan: ${escapeHtmlText(planName)}<br />Included callers: ${params.includedCallers}<br />Callers used: ${params.capturedCallers}<br />Overage callers: ${params.overageCallers}<br />Overage rate: $0.25/caller<br />Overage charge: $${money(params.amountCents)}</p>`,
