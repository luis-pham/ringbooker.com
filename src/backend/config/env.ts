...
function createValidatedEnv() {
  return createEnv({
    server: {
      ...
      AGENT_RUNTIME_MODE: z.enum(['mock', 'livekit_gemini', 'livekit_native_gemini', 'livekit_openai']).default('mock'),
      ...
      EMAIL_FROM_ADDRESS: z.string().email().default('no-reply@ringbooker.local'),
