...
export const RUNTIME_CONFIG_TEMPLATE = [
  ...
  "- SHOP STATUS HOURS: today's open/close time for open/closed status checks only. Not for weekly schedule answers.",
  '- WEEKLY SCHEDULE: full weekly hours. Use this when caller asks what your hours are or when you are open.',
...
export function renderRuntimeBusinessConfig(config: RuntimeBusinessConfig): string {
  ...
  return [
    ...
    `SHOP STATUS HOURS: ${config.todayHours ? compactPromptLine(config.todayHours, 80) : 'Not specified'} (current session only -- do not use to answer weekly schedule questions)`,
    config.hours ? `WEEKLY SCHEDULE (use this when caller asks about hours, schedule, or when you are open): ${compactPromptLine(config.hours, 700)}` : null,
