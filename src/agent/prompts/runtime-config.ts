...
export const STATIC_SERVICE_SCOPE_RULES = [
  ...
  '- If a caller has clear booking intent AND asks for a broad service group, do not read a menu. Treat the group as service context and ask for date/time first unless the exact service is required before the next step. Do NOT ask which specific service for any information question -- answer at the group level instead.',
  ...
  '- If a caller names a provider or stylist, acknowledge that provider by name before asking any service, date, or time question.',
].join('\n');
...
export const RUNTIME_CONFIG_TEMPLATE = [
  ...
  "- SHOP STATUS HOURS: today's open/close for status only. Not for answering weekly schedule questions.",
  '- WEEKLY SCHEDULE: full week Mon-Sun in AM/PM format. Use this when caller asks about hours or schedule.',
  ...
  '- When services are grouped, use the group as context. For booking intent, ask which specific service item only if it is required before the next step; otherwise ask for date/time first and clarify the exact item later if needed. For all other queries -- information, pricing, availability, hours, policies, comparisons, or anything not explicitly a booking request -- answer at the group level without narrowing down.',
