...
test('renderServicePrice treats a defined $0 price as free, not "varies"', () => {
  assert.equal(renderServicePrice({ name: 'Balayage Consultation', price: 0, priceType: undefined }), 'free');
});

test('renderServicePrice still honors an explicit "varies" priceType even when price is 0', () => {
  assert.equal(renderServicePrice({ name: 'Balayage Consultation', price: 0, priceType: 'varies' }), 'price varies');
});
...
test('renderRuntimeOptional renders "free" for a $0 service inside the actual composed prompt text', () => {
  const config: RuntimeBusinessConfig = {
    ...
    services: [
      { name: 'Balayage Consultation', category: 'Color', price: 0, duration: '20 min' },
  ...
  assert.match(rendered, /Balayage Consultation \| free \| 20 min/);
